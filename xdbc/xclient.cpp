...
namespace xdbc
{
                                        ...
                                        _baseSocket(_ioContext)
    {
        ...
        spdlog::get("XDBC.CLIENT")->info("Creating Client: {0}, BPS: {1}, BS: {2} bytes, TS: {3} bytes, iformat: {4} ", _xdbcenv->env_name, env.buffers_in_bufferpool, env.buffer_size, env.tuple_size, env.iformat);
            ...
            available_buffers_for_queues < total_consumer_threads)
        {
            ...
            spdlog::get("XDBC.CLIENT")->error("Buffer allocation error: Total buffers: {0}. "
                                              ...
                                              _xdbcenv->buffers_in_bufferpool, total_producer_threads, total_consumer_threads, total_producer_threads + total_consumer_threads, total_producer_threads, available_buffers_for_queues, total_consumer_threads, total_producer_threads + total_consumer_threads);
        }
        ...
        // }
        _xdbcenv->bufferGen.setBufferSize(_xdbcenv->buffer_size);
        _xdbcenv->bufferGen.add_freeBuffers(_xdbcenv->freeBufferIds, env.buffers_in_bufferpool);
    ...
    void XClient::receive(int thr)
    {
        ...
        if (connected)
        {
            ...
            while (error != boost::asio::error::eof)
            {
                ...
                std::vector<std::byte> &newBuf = _xdbcenv->bufferGen.createBuffer(bpi, (sizeof(Header) + (_xdbcenv->tuples_per_buffer * _xdbcenv->tuple_size)));
                ...
                headerBytes = boost::asio::read(socket, boost::asio::buffer(newBuf.data(), sizeof(Header)),
                                                boost::asio::transfer_exactly(sizeof(Header)), error);
                Header header = *reinterpret_cast<Header *>(newBuf.data());
                ...
                readBytes = boost::asio::read(socket, boost::asio::buffer(newBuf.data() + sizeof(Header), header.totalSize),
    ...
    void XClient::decompress(int thr)
    {
        ...
        while (true)
        {
            ...
            if (compBuffId == -1)
                break;
            std::vector<std::byte> &compBuf = _xdbcenv->bufferGen.getBuffer(compBuffId);
            _xdbcenv->pts->push(ProfilingTimestamps{std::chrono::high_resolution_clock::now(), thr, "decomp", "pop"});

            Header *header = reinterpret_cast<Header *>(compBuf.data());
            std::byte *compressed_buffer = compBuf.data() + sizeof(Header);
            ...
            else if (header->compressionType > 0)
            {
                ...
                std::vector<std::byte> &decompBuf = _xdbcenv->bufferGen.createBuffer(decompBuffId, compBuf.size());
                ...
                else
                {
                    ...
                    memcpy(decompBuf.data(), &newHeader, sizeof(Header));
    ...
    buffWithId XClient::getBuffer(int readThreadId)
    {
        ...
        if (buffId > -1)
        {
            std::vector<std::byte> &decompBuf = _xdbcenv->bufferGen.getBuffer(buffId);
            auto header = reinterpret_cast<Header *>(decompBuf.data());
            totalTuples = header->totalTuples;
            totalSize = header->totalSize;
            curBuf.buff = decompBuf.data() + sizeof(Header);
        }
        curBuf.id = buffId;
