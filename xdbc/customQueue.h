...
public:
    ...
    {
        static size_t lastCopiedIndex = 0;
        std::vector<T> new_elements;
        auto current_index = this->d_queue.size(); // Use this->d_queue

        if (lastCopiedIndex < current_index)
        {
            new_elements.assign(this->d_queue.rbegin(), // Use this->d_queue
                                this->d_queue.rbegin() + (this->d_queue.size() - lastCopiedIndex));
            lastCopiedIndex = current_index;
        }
        return new_elements;
    }
};
